...
from keras_hub.src.models.tinyllama.tinyllama_decoder import (
    TinyllamaTransformerDecoder,
)
from keras_hub.src.models.tinyllama.tinyllama_layernorm import (
    TinyllamaLayerNorm,
)


def _tinyllama_kernel_initializer(stddev=0.02):
    return keras.initializers.RandomNormal(stddev=stddev)


@keras_hub_export("keras_hub.models.TinyllamaBackbone")
class TinyllamaBackbone(Backbone):
    """
    The Tinyllama Transformer core architecture with hyperparameters.
    ...
    # Pretrained Tinyllama decoder.
    model = keras_hub.models.TinyllamaBackbone.from_preset("TinyLlama_v1.1")
    model(input_data)

    # Randomly initialized Llama decoder with custom config.
    model = keras_hub.models.TinyllamaBackbone(
        vocabulary_size=10,
        hidden_dim=512,
        num_layers=2,
        num_query_heads=32,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.token_embedding = ReversibleEmbedding(
            ...
            embeddings_initializer=_tinyllama_kernel_initializer(stddev=0.01),
            dtype=dtype,
            name="token_embedding",
        )
        self.transformer_layers = []
        for i in range(num_layers):
            layer = TinyllamaTransformerDecoder(
                ...
                activation=ops.silu,
                kernel_initializer=_tinyllama_kernel_initializer(stddev=0.02),
                dropout=dropout,
                dtype=dtype,
                name=f"transformer_layer_{i}",
            )
            self.transformer_layers.append(layer)
        self.layer_norm = TinyllamaLayerNorm(
